...
export function middleware(request: NextRequest) {
  ...
  // 2️⃣ External Admin Redirect
  // FIX: Enforce that it ONLY redirects if the request is traveling internally on the frontend site, 
  // ensuring external API calls to 'admin.uptimiseit.com' are left completely untouched!
  if (url.pathname.startsWith('/admin') && !url.hostname.includes('admin.')) {
    return NextResponse.redirect('https://admin.uptimiseit.com/admin/login', 301);
  }
