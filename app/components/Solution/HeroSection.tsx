...
// components/solutions/HeroSection.tsx
"use client";
...
export default function HeroSection({ data, slug }: { data: any, slug: string }) {
  // Use the slug to define a unique "Accent Color" for this page
  const themeColor = slug === 'mvp-development' ? '#2563eb' : 
                     slug === 'ai-products' ? '#9333ea' : '#10b981';
    ...
    <section className="relative pt-32 pb-20 px-6 lg:px-12 max-w-[1440px] mx-auto overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <motion.div 
          ...
          className="space-y-8 relative z-10"
        >
          <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter text-slate-900 leading-[0.85]">
            {data.heading} <br />
            <span style={{ color: themeColor }} className="italic font-serif">
              {data.accent}
            </span>
          </h1>
          ...
          <button 
            style={{ backgroundColor: themeColor }}
            className="px-10 py-5 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:scale-105 transition-transform"
          >
            Initialize_Project
          </button>
        </motion.div>

        {/* RIGHT ANIMATION: Controlled by Slug */}
        <div className="relative h-[500px] flex items-center justify-center">
          <HeroVisuals slug={slug} themeColor={themeColor} />
        </div>
      </div>
    </section>
  );
