    ...
    const initData = async () => {
      try {
        ...
        const [catRes, blogRes] = await Promise.all([
          fetch("https://uptimiseit-admin.vercel.app/api/categories"),
          fetch("https://uptimiseit-admin.vercel.app/api/blogs")
        ]);
        ...
        if (catJson.success) {
          ...
          if (catJson.data.length > 0) setActiveCategory(catJson.data[0]);
        }
        if (blogJson.success) setBlogData(blogJson.data);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end mb-16 gap-8">
          <div className="max-w-2xl space-y-6 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/5 border border-blue-500/10">
              ...
              <span className="text-[11px] font-black uppercase tracking-[0.2em] text-blue-400 font-mono">Stream_Protocol::v1.0</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-4">
            <p className="text-[11px] font-black font-mono text-slate-500 uppercase tracking-[0.4em] mb-8 px-4 flex items-center gap-2">
            ...
            {loading ? (
              ...
              allCategories.map((cat) => (
                <button 
                  ...
                  aria-label={`Switch to ${cat.name} category`}
                ...
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className={`text-[11px] font-medium leading-relaxed line-clamp-1 ${activeCategory?.id === cat.id ? "text-blue-100" : "text-slate-500"}`}>{cat.description || "Operational intelligence."}</p>
                    </div>
                  </div>
                </button>
              ))
          ...
          <div className="lg:col-span-8 space-y-8">
              ...
              {loading ? (
                ...
                <AnimatePresence mode="wait">
                  {displayBlogs.map((post: any) => (
                    <motion.div key={post.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }}>
                      <Link href={`/blog/${activeCategory?.slug}/${post.slug}`} aria-label={`Read article: ${post.title}`}>
                        <div className="group relative p-12 h-full rounded-[4rem] bg-white flex flex-col justify-between overflow-hidden hover:shadow-[0_30px_60px_-15px_rgba(37,99,235,0.15)] transition-all duration-700">
                          ...
                          <div className="space-y-10 relative z-10">
                            <div className="flex justify-between items-center">
                              <span className="text-[11px] font-black font-mono text-blue-700 uppercase tracking-widest bg-blue-50 px-4 py-2 rounded-xl border border-blue-100">
                            ...
                            <div className="space-y-3">
                              <p className="text-[11px] font-black text-slate-600 uppercase tracking-[0.3em]">Log::{activeCategory?.name}</p>
                          ...
                          <div className="mt-12 flex items-center gap-3 text-[11px] font-black font-mono text-slate-500 group-hover:text-blue-600 transition-all uppercase tracking-widest">
                ...
                <div className="md:col-span-2 p-24 text-center border-2 border-dashed border-white/10 rounded-[4rem] flex flex-col items-center justify-center gap-4">
                  <Loader2 size={24} className="text-slate-700" />
            ...
            {/* SYNC Newsletter */}
            <div className="p-12 rounded-[4rem] bg-[#0f172a] border border-white/5 text-white flex flex-col lg:flex-row items-center justify-between gap-12 relative overflow-hidden">
              ...
              <form onSubmit={handleSubscribe} className="relative z-10 flex flex-col w-full lg:w-auto">
                ...
                {message && <p className="text-[11px] mt-4 font-black uppercase text-emerald-400 tracking-widest text-center lg:text-left">{message}</p>}
              </form>
            </div>
          </div>
        </div>
