    ...
    <section className="relative bg-gradient-to-br from-slate-950 to-slate-900 py-24 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="max-w-2xl space-y-6">
            <motion.div 
              initial={{ scale: 0.8, opacity: 0 }}
              whileInView={{ scale: 1, opacity: 1 }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 shadow-sm"
            >
              <Eye className="text-emerald-400 animate-pulse" size={14} />
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 font-mono">
                Visibility Protocol :: Active
              </span>
            </motion.div>
            <h2 className="text-4xl md:text-6xl font-black font-header tracking-tighter text-white leading-[0.85] uppercase">
              Full Spectrum <br />
              <motion.span 
          ...
          <p className="text-lg text-slate-400 font-body max-w-sm lg:text-right font-medium leading-relaxed">
            No black boxes. No hidden cycles. Just pure engineering visibility from discovery to deployment.
          </p>
        </motion.div>
        ...
        >
          {transparencyPoints.map((item, i) => (
            <motion.div
              ...
              className={`p-8 rounded-[3rem] border transition-all duration-500 group relative flex flex-col justify-between overflow-hidden cursor-default ${
                item.featured 
                ? 'bg-slate-800/80 border-slate-700 text-white shadow-2xl shadow-emerald-900/20' 
                : 'bg-slate-900/40 backdrop-blur-md border-slate-800 hover:border-emerald-500/30 hover:bg-slate-800/80 hover:shadow-2xl hover:shadow-emerald-900/20'
              } ${item.size}`}
            >
              <div className="relative z-10 space-y-8">
                <div className="flex justify-between items-center">
                  <motion.div 
                    // variants={iconFloating}
                    animate="animate"
                    className={`p-4 rounded-2xl border ${item.featured ? 'bg-emerald-600 border-emerald-500 text-white shadow-lg shadow-emerald-900/50' : 'bg-slate-800 border-slate-700 text-emerald-400 shadow-sm group-hover:bg-emerald-600 group-hover:border-emerald-500 group-hover:text-white transition-colors'}`}
                  >
                    {item.icon}
                  </motion.div>
                  <div className={`px-3 py-1 rounded-full border text-[9px] font-mono font-black tracking-widest uppercase ${
                    item.featured ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-slate-950/50 border-slate-800 text-slate-400'
                  }`}>
                    {item.status}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-black font-header tracking-tight uppercase text-white">
                    {item.title}
                  </h3>
                  <p className="text-base font-body leading-relaxed max-w-md font-medium text-slate-400">
                    {item.desc}
                  </p>
                </div>
              </div>
              ...
              <div className="relative z-10 flex justify-end mt-8">
                <motion.div 
                  initial={{ opacity: 0, x: -10 }}
                  whileHover={{ x: 0 }}
                  className={`flex items-center gap-2 text-[10px] font-black font-mono tracking-[0.2em] opacity-0 group-hover:opacity-100 transition-all ${item.featured ? 'text-emerald-400' : 'text-emerald-400'}`}
                >
                  VIEW PROTOCOL <ChevronRight size={12} className="group-hover:translate-x-1 transition-transform" />
                </motion.div>
              </div>
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 0.5, y: 0 }}
          transition={{ duration: 1, delay: 0.5 }}
          className="mt-24 flex flex-wrap justify-center gap-12 items-center grayscale opacity-70 hover:opacity-100 transition-opacity"
        >
          {[
            { icon: <ShieldCheck className="text-emerald-500" />, text: "IP_PROTECTION" },
            { icon: <Github className="text-white" />, text: "TOTAL_OWNERSHIP" },
            { icon: <Radio className="text-blue-400" />, text: "LIVE_MONITORING" }
          ].map((trust, idx) => (
            <React.Fragment key={trust.text}>
              <motion.div 
                whileHover={{ opacity: 1, scale: 1.05 }}
                className="flex items-center gap-3 text-slate-300 font-black tracking-tighter text-2xl cursor-default hover:text-white transition-colors"
              >
                {trust.icon}
                {trust.text}
              </motion.div>
              {idx < 2 && <div className="h-4 w-px bg-slate-800 hidden md:block" />}
            </React.Fragment>
          ))}
        </motion.div>
...
export default TransparencySectionDark;