...
import { 
  Boxes, ChevronRight, Loader2, ArrowUpRight, 
...
const ProjectsLedger = () => {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="max-w-2xl space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10">
              <Boxes className="text-blue-500 animate-pulse" size={14} />
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 font-mono">
                Project_Ledger::2026
              </span>
            </div>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-white leading-[0.85] uppercase">
              Proven <br />
              <span className="text-blue-500 italic">Deployments.</span>
            </h2>
          </div>
          <p className="text-lg text-slate-400 max-w-sm lg:text-right font-medium leading-relaxed">
            A small, high-impact selection of the world-class products we've engineered.
          </p>
        </motion.div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 font-mono">Syncing_Deployment_Nodes...</p>
          </div>
        ) : (
          <motion.div 
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
          ...
          >
            {projects.map((project, i) => {
              const isLarge = i === 0; // First item is the large featured card
              
              return (
                <motion.div
                  key={project.id}
                  whileHover={{ y: -10 }}
                  className={`group relative rounded-[3rem] border transition-all duration-500 overflow-hidden cursor-pointer flex flex-col justify-between p-10 ${
                    isLarge 
                    ? 'lg:col-span-2 min-h-[500px] border-white/10' 
                    : 'min-h-[400px] bg-white border-slate-100 hover:shadow-2xl hover:shadow-blue-500/10'
                  }`}
                >
                  ...
                  <div className="relative z-10 h-full flex flex-col justify-between">
                    <div className="space-y-6">
                      <div className="flex justify-between items-center">
                        <span className={`text-[10px] font-black tracking-widest uppercase ${isLarge ? 'text-blue-400' : 'text-slate-500'}`}>
                          {project.tag}
                        </span>
                        <div className="px-3 py-1 rounded-full border border-white/10 bg-white/5 text-[9px] font-black text-slate-400 uppercase font-mono">
                          System_Live
                        </div>
                      </div>

                      <div className="space-y-4">
                        <h3 className={`font-black tracking-tighter leading-none uppercase ${isLarge ? 'text-4xl md:text-6xl text-white' : 'text-3xl text-slate-900'} group-hover:text-blue-500 transition-colors`}>
                          {project.title}
                        </h3>
                        <p className={`text-sm md:text-base font-medium leading-relaxed line-clamp-3 ${isLarge ? 'text-slate-300 max-w-xl' : 'text-slate-500'}`}>
                          {project.description}
                        </p>
                      </div>
                    </div>

                    <div className={`mt-10 flex justify-between items-end border-t pt-8 ${isLarge ? 'border-white/10' : 'border-slate-100'}`}>
                      <div className="space-y-2">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                          <Code2 size={12} /> Tech_Stack
                        </span>
                        <p className={`text-xs font-bold uppercase ${isLarge ? 'text-blue-200' : 'text-slate-700'}`}>
                          {project.technology}
                        </p>
                      </div>
  ...
  aria-label={`Visit ${project.title} live website`}>
            ...
            >
              ...
              <Link href="/case-studies" className="flex items-center gap-2 text-[10px] font-black font-mono text-white uppercase cursor-pointer hover:underline">
                View Full Ledger <ChevronRight size={14} />
              </Link>
            </motion.div>
