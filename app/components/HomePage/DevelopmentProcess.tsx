...
const DevelopmentProcess = () => {
  return (
    <section className="bg-gradient-to-br from-slate-950 to-slate-900 py-24 px-6 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-white uppercase leading-none">
            Engineering <br /><span className="text-blue-400 italic">Lifecycle.</span>
          </h2>
          <div className="flex flex-col items-center lg:items-end gap-2">
            <span className="text-blue-400 font-mono text-xs font-black tracking-[0.3em] uppercase">
              # The Factory Protocol
            </span>
            <div className="h-px w-24 bg-blue-900/50 hidden lg:block" />
          </div>
        </motion.div>
        ...
        <div className="relative">
          ...
          <motion.div 
            ...
            whileInView="visible"
            viewport={{ once: true }}
            className="absolute left-6 top-0 w-[2px] bg-slate-800 hidden md:block origin-top" 
          />
            ...
            {steps.map((step, i) => (
              ...
              >
                ...
                <div className="relative z-10 flex-shrink-0 mt-1">
                  <motion.div 
                    whileHover={{ scale: 1.2, rotate: 90 }}
                    className={`w-12 h-12 rounded-2xl ${step.color} text-white flex items-center justify-center shadow-xl shadow-black/40 transition-all duration-300`}
                  >
                    {step.icon}
                  </motion.div>
                </div>
                ...
                <div className="flex-grow pb-2">
                  <motion.div 
                    whileHover={{ x: 10 }}
                    className="p-6 rounded-[2rem] bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 hover:bg-slate-800 hover:shadow-2xl hover:shadow-black/50 transition-all duration-500 hover:border-blue-500/30 cursor-default"
                  >
                    <div className="flex justify-between items-center mb-2">
                       <h3 className="text-2xl font-bold text-slate-100 tracking-tight flex items-center gap-3">
                        <span className="text-xs font-mono text-slate-500 opacity-50">/{step.number}</span>
                        {step.title}
                      </h3>
                      <span className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-widest bg-slate-900/80 px-2 py-1 rounded-md">
                        Phase_Active
                      </span>
                    </div>
                    <p className="text-slate-400 font-body text-sm max-w-2xl leading-relaxed">
                      {step.desc}
                    </p>
                  </motion.div>
                </div>
        ...
        <motion.div 
          ...
          className="mt-16 flex justify-center md:justify-start md:ml-20"
        >
          <div className="inline-flex items-center gap-2 text-blue-400 font-bold text-sm uppercase tracking-widest group cursor-pointer">
            <Workflow size={16} className="animate-spin-slow" />
            <span>Operationalizing Success</span>
          </div>
        </motion.div>
