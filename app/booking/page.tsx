...
import { 
  ShieldCheck, Bot, Cpu, Zap, ChevronRight, 
  Linkedin, Command, Globe, Check, Sparkles, Loader2
} from 'lucide-react';
import Link from 'next/link';
...
const BookingPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://uptimiseit-admin.vercel.app/api/intake', {
        method: 'POST',
        headers: {
  ...
  return (
    <main className="min-h-screen bg-[#FDFDFF] font-sans pt-20 selection:bg-blue-100 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-20 pb-32 relative z-10">
        ...
        {/* --- LEFT: Technical Value Stage --- */}
        <motion.div variants={containerVars} initial="hidden" animate="visible" className="lg:col-span-5 text-center lg:text-left space-y-16 py-12">
          <div className="space-y-6">
            <motion.div variants={itemVars} className="p-3 bg-blue-50 rounded-2xl w-fit text-blue-600">
                <Sparkles size={24} />
            </motion.div>
            <motion.h1 variants={itemVars} className="text-6xl md:text-8xl font-black font-header tracking-tighter text-slate-950 leading-[0.85] uppercase">
              Elite <br />
              <span className="text-blue-600 italic">Access.</span>
            </motion.h1>
            <motion.p variants={itemVars} className="text-xl text-slate-500 font-body leading-relaxed max-w-sm font-medium">
              We don&apos;t do &quot;sales calls.&quot; We do architecture audits. Prepare for a deep-dive into your system&apos;s potential.
            </motion.p>
          </div>
        ...
        <div className="lg:col-span-7">
          ...
          >
            ...
            <div className="p-12 border-b border-slate-50 bg-[#FDFDFF] relative overflow-hidden">
                ...
                <h3 className="text-3xl font-black text-slate-950 tracking-tighter uppercase">PROJECT_INTAKE_v1.0</h3>
                <p className="text-slate-400 text-sm font-bold uppercase tracking-widest mt-2 flex items-center gap-2">
                    <Globe size={14} className="text-blue-500" /> Secure Engineering Gateway
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-12 space-y-10">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Brief Project Context</label>
                    <span className="text-[9px] font-mono font-bold text-slate-300">{formData.projectContext.length}/300</span>
                </div>
                <textarea 
                  maxLength={300}
              ...
              <div className="pt-6">
                <button 
                  ...
                  className={`w-full h-20 rounded-3xl font-black uppercase tracking-[0.2em] text-xs transition-all duration-500 shadow-2xl flex items-center justify-center gap-4 group disabled:opacity-50 ${
                    status === 'success' ? 'bg-emerald-500 text-white' : 'bg-slate-950 text-white hover:bg-blue-600 shadow-blue-900/10'
                  }`}
                >
                  {isSubmitting ? (
                    <div className="flex items-center gap-3">
                ...
                {status === 'error' && (
                  <p className="text-red-500 text-[10px] font-bold uppercase tracking-widest text-center mt-4 font-mono">Critical_Error: Transmission_Failed</p>
                )}
              </div>
            </form>
