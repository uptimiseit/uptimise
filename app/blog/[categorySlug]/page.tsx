...
export default function DynamicCategoryBlogPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch Categories to find the ID associated with this URL slug
        ...
        if (blogJson.success) {
          // 3. FILTER BY categoryId (Highly Reliable)
          // We convert both to Number to ensure a perfect match
          const filteredByCat = blogJson.data.filter(
            (post: any) => Number(post.categoryId) === Number(currentCat.id)
          );

          setAllPosts(filteredByCat);

          // 4. Extract Unique Sub-Categories
          const uniqueSubCats = Array.from(
            new Set(
              filteredByCat
                .map((post: any) => post.subCategory)
                .filter((sub: any) => sub && sub.trim() !== "")
            )
          ) as string[];
  ...
  // Client-side Filtering for Search and Sub-Tabs
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFDFF]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Loading_Nodes...</p>
        </div>
      </div>
    );
  }
      ...
      <section className="pt-40 pb-16 px-6 max-w-7xl mx-auto text-center space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium max-w-xl mx-auto text-sm md:text-base">
            {categoryInfo?.description || `Technical insights and strategic thinking in ${categoryInfo?.name}.`}
          </p>
        </motion.div>
      </section>
        ...
        <div className="sticky top-[80px] z-30 bg-white/80 backdrop-blur-md py-4 border-b border-slate-100 mb-10 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-3 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
            />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            {filteredPosts.length === 0 ? (
                <div className="p-24 text-center border-2 border-dashed border-slate-100 rounded-[3rem] text-slate-300 font-bold uppercase tracking-widest text-[10px]">
                      ...
                      <motion.div key={post.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <Link href={`/blog/${categorySlug}/${post.slug}`}>
                          <div className={`bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all group ${viewMode === "list" ? "md:flex h-full" : ""}`}>
                            <div className={`${viewMode === "list" ? "w-full md:w-2/5 h-64 md:h-auto" : "w-full h-56"} bg-slate-50 relative overflow-hidden`}>
                              {post.featuredImage ? (
                            ...
                            <div className="p-8 flex flex-col flex-1">
                                ...
                                <div className="mt-auto pt-6 border-t border-slate-50 flex justify-between items-center">
                                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-tighter">Engineered_By {post.author}</span>
          ...
          <div className="lg:col-span-4">
            <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm sticky top-[150px]">
              <h3 className="text-[10px] font-black text-slate-950 mb-8 flex items-center gap-2 uppercase tracking-[0.3em]">
                <TrendingUp size={14} className="text-blue-600" /> Trending_Insights
              </h3>
              <div className="space-y-8">
                {popularPosts.map((post, idx) => (
                  <Link key={post.id} href={`/blog/${categorySlug}/${post.slug}`} className="flex gap-4 group">
