...
import { 
  ...
  ChevronRight, ChevronLeft, Mail, BookOpen
} from "lucide-react";

// --- MOCK DATA ---
const CATEGORIES = [
  "All Articles", "AI & Automation", "Software Development", 
  "Product Design", "DevOps & Cloud", "Digital Growth", "Startup Strategy"
];
...
const MOCK_POSTS = [
  {
    ...
    excerpt: "How AI-assisted engineering workflows are transforming how modern digital products are built, tested, and deployed at venture speed.",
  ...
  },
  {
    ...
    excerpt: "A comprehensive guide to designing high-retention SaaS systems using event-driven microservices and cloud-native infrastructure.",
  ...
  },
  {
    ...
    excerpt: "Eliminate deployment bottlenecks and achieve 99.99% uptime by automating your CI/CD pipelines and infrastructure as code.",
  ...
  },
  {
    ...
    excerpt: "Why standard web design fails for enterprise software, and how to use cognitive load reduction to build intuitive B2B interfaces.",
  ...
  },
  {
    ...
    excerpt: "Analyzing the performance, SEO, and conversion rate benefits of migrating legacy monoliths to headless Next.js architectures.",
    category: "Software Development",
    date: "Sep 28, 2026",
    readTime: "7 min read",
    featured: false,
  ...
  },
  {
    ...
    excerpt: "A deep dive into how we utilize autonomous AI agents to scaffold, write, and test boilerplate code 40% faster than traditional agencies.",
    category: "AI & Automation",
    date: "Sep 22, 2026",
  ...
  },
  {
    ...
    excerpt: "Founders waste months building features users don't want. Here is the exact framework to isolate your core value prop and launch in 6 weeks.",
    ...
    <div className="min-h-screen bg-[#FDFDFF] text-slate-900 font-sans pb-24">
      ...
      <section className="pt-32 pb-16 px-6 max-w-7xl mx-auto text-center space-y-6">
        ...
        <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight text-slate-900 max-w-4xl mx-auto">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500">Technology Trends</span>
        </h1>
        <p className="text-lg text-slate-600 max-w-3xl mx-auto leading-relaxed">
          Written by engineers, designers, and technology specialists. We share actionable insights on AI-native engineering, cloud infrastructure, and product strategy to help you build better digital platforms.
        </p>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {activeCategory === "All Articles" && searchQuery === "" && featuredPost && (
          <Link href={'/blog/blog-detail'} className="mb-16 group cursor-pointer">
          <div className="mb-16 group cursor-pointer">
            <div className="text-xs font-black tracking-widest text-indigo-600 uppercase mb-4 flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse"></div> Featured Article
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
            <div className={
              ...
              {filteredPosts.length > 0 ? filteredPosts.map(post => (
                ...
                <div key={post.id} className={`bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:border-indigo-200 transition-all group cursor-pointer ${viewMode === "list" ? "md:flex items-center" : "flex flex-col"}`}>
                  ...
                  <div className={`p-6 flex flex-col ${viewMode === "list" ? "md:w-2/3 md:justify-center" : "flex-1"}`}>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">{post.category}</span>
                ...
                <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-2xl">
                  ...
                  <button onClick={() => {setSearchQuery(""); setActiveCategory("All Articles");}} className="mt-4 text-indigo-600 font-bold hover:underline">Clear Filters</button>
                </div>
              )}
            </div>
            ...
            {filteredPosts.length > 0 && (
              <div className="flex items-center justify-center gap-2 mt-16 pt-8 border-t border-slate-200">
                <button className="p-2 text-slate-400 hover:text-indigo-600 transition-colors"><ChevronLeft size={20}/></button>
                <button className="w-10 h-10 rounded-lg bg-indigo-600 text-white font-bold shadow-md">1</button>
                <button className="w-10 h-10 rounded-lg bg-white border border-slate-200 text-slate-600 font-bold hover:border-indigo-300 transition-colors">2</button>
                <button className="w-10 h-10 rounded-lg bg-white border border-slate-200 text-slate-600 font-bold hover:border-indigo-300 transition-colors">3</button>
                <span className="text-slate-400 font-bold">...</span>
                <button className="w-10 h-10 rounded-lg bg-white border border-slate-200 text-slate-600 font-bold hover:border-indigo-300 transition-colors">8</button>
                <button className="p-2 text-slate-400 hover:text-indigo-600 transition-colors"><ChevronRight size={20}/></button>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4 space-y-10">
              ...
              <div className="space-y-6">
                {popularPosts.map((post, idx) => (
                  <div key={post.id} className="flex gap-4 group cursor-pointer">
                    <span className="text-3xl font-black text-slate-200 group-hover:text-indigo-100 transition-colors">0{idx + 1}</span>
                    <div>
            ...
            <div className="bg-slate-900 rounded-3xl p-8 shadow-xl relative overflow-hidden">
              ...
              <h3 className="text-xl font-extrabold text-white mb-2 relative z-10">Stay Updated</h3>
              <p className="text-slate-400 text-sm leading-relaxed mb-6 relative z-10">
                Subscribe to receive new articles about technology, product development, and digital innovation.
              </p>
              ...
              <p className="text-center text-[10px] text-slate-500 mt-4 relative z-10">No spam. Unsubscribe anytime.</p>
            </div>

            {/* Sidebar Categories (Desktop Only) */}
            <div className="hidden lg:block bg-slate-50 border border-slate-200 rounded-3xl p-8">
              <h3 className="text-lg font-extrabold text-slate-900 mb-4">Topics</h3>
